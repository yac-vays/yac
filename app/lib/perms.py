...
async def get_from_roles(
    op: inp.OperationRequest, specs: spc.Specs, old_data: dict
) -> list[str]:
    """
            ...
            if rtest:
                ...
                if role_def.startswith(f"all:{op.type_name}:"):
                    ...
                    try:
                        ...
                        logger.error(
                            f"Set {op.type_name}.{set_name} could not be rendered:"
                            f" {error}"
                        )
                        stest = False
                    if stest:
                        perms.append(perm)
                elif role_props["name"] is not None and role_def.startswith(
                    f"{op.type_name}:{role_props['name']}:"
                ):
                    perms.append(role_def.split(":", 2)[2])
